...
const Editmovies = () => {
  ...
  const [updatedName,setUpdatedImg]=useState("")
...
console.log(res.data)
      if (res.data.result) {
        ...
        setMoviesDeatils({
          ...
          image:res.data.singleMovieDetails.image,
  ...
  const submitted = async (e) => {
    try {
      ...
      const response = await updateTheMovieDetails(token, movieid, moviesDetails.movieName, moviesDetails.yeasOfRealease, moviesDetails.actors, moviesDetails.producer,moviesDetails.image)
  ...
  setUpdatedImg(image.name)
    const reader=new FileReader;
reader.readAsDataURL(image)
    reader.onload=()=>{
      setMoviesDeatils({...moviesDetails,image:reader.result})
    }
...
<div className='pe-4 ps-4 pt-2 pb-4'  style={{ border: "1px solid #DDDDDD" }}>
          ...
          <div>
            <Form onSubmit={submitted}>
              ...
              <label onChange={(e) => uploadImage(e.target.files[0])} style={{ width: "100%" }} className='btn  mb-2 btn-outline-primary'>
                {updatedName ?`${updatedName.substring(0,9)}..img`:(moviesDetails&&`${moviesDetails.movieName.substring(0,9)}...img`)}
                <Input
